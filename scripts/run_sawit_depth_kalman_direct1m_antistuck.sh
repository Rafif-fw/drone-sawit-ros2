...
RUN_ID="${1:-depth_direct1m_antistuck_01}"
...
echo "Mode          : Depth setiap frame + ToF setiap pesan + Kalman"
echo "Visit         : langsung sekitar 1 m, tanpa 3 m dan 2 m"
echo "Run ID        : $RUN_ID"
...
exec ros2 run sawit_autonomy \
  sawit_navigator_depth_kalman_direct1m_antistuck \
  --ros-args \
  ...
  -p normal_csv_path:="$HOME/ros2_ws/src/sawit_autonomy/data/depth_direct1m_antistuck_visits.csv" \
  -p tof_kalman_csv_path:="$HOME/ros2_ws/src/sawit_autonomy/data/depth_direct1m_antistuck_tof_updates.csv" \
  -p depth_kalman_csv_path:="$HOME/ros2_ws/src/sawit_autonomy/data/depth_direct1m_antistuck_depth_updates.csv" \
  -p depth_every_message_kalman_enabled:=true \
  -p tof_kalman_every_msg_enabled:=true \
  -p depth_kalman_association_gate:=1.35 \
  -p depth_kalman_bearing_gate_deg:=18.0 \
  -p depth_kalman_range_gate:=2.00 \
  -p depth_kalman_measurement_std:=0.28 \
  -p depth_kalman_max_speed:=0.80 \
  -p depth_kalman_min_score:=150.0 \
  -p depth_kalman_require_strong_while_moving:=true \
  -p depth_kalman_log_each_update:=false \
  -p tof_kalman_log_each_update:=false \
  -p direct_visit_distance:=1.00 \
  -p direct_visit_tolerance:=0.10 \
  -p direct_visit_min_safe:=0.82 \
  -p direct_visit_max_mad:=0.15 \
  -p direct_visit_min_samples:=5 \
  -p direct_visit_hold_time:=0.80 \
  -p reset_memory_on_start:=true \
  -p debug_pc_enabled:=true \
  -p debug_publish_period:=1.0
