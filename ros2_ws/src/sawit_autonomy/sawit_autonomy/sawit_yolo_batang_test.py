...
class YoloBatangTest(Node):
    def __init__(self) -> None:
        ...
        self.get_logger().info("START YOLO-BATANG TEST (deteksi saja, drone tidak digerakkan)")
    ...
    def xyz_from_cloud_pixels(
        ...
        if int(cloud.height) <= 1 or int(cloud.width) <= 1:
            return None
        if int(cloud.height) != image_h or int(cloud.width) != image_w:
            return None
    ...
    def xyz_from_depth(
        ...
        if self.latest_depth is None:
            return None
        depth = self.decode_depth(self.latest_depth)
        if depth is None:
            return None
        h, w = image_shape
    ...
    def process_once(self) -> None:
        ...
        self.busy = True
        try:
            frame = self.decode_image(self.latest_image)
            h, w = frame.shape[:2]
            ...
            if not results:
                self.publish_debug(frame, self.latest_image.header)
                self.update_tracks([])
                return
            ...
            if boxes is not None:
                ...
                for idx, (box, conf, cls_id) in enumerate(zip(xyxy, confs, classes)):
                    x0 = max(0, min(w - 1, int(round(float(box[0])))))
                    y0 = max(0, min(h - 1, int(round(float(box[1])))))
                    x1 = max(x0 + 1, min(w, int(round(float(box[2])))))
                    y1 = max(y0 + 1, min(h, int(round(float(box[3])))))
                    bbox = (x0, y0, x1, y1)

                    seg = mask_data[idx] if mask_data is not None and idx < len(mask_data) else None
                    pixel_mask = self.make_detection_mask(bbox, (h, w), seg)

                    xyz = None
                    source = "none"
                    if self.latest_cloud is not None and time.monotonic() - self.latest_cloud_time <= 1.0:
                        xyz = self.xyz_from_cloud_pixels(self.latest_cloud, pixel_mask, (h, w))
                        if xyz is not None:
                            source = "organized_pointcloud"
                    if xyz is None:
                        xyz = self.xyz_from_depth(bbox, (h, w))
                        if xyz is not None:
            ...
            if now - self.last_summary_time >= 1.0:
                ...
                if detections:
                    ...
                    self.get_logger().info(
                        f"YOLO_BATANG detections={len(detections)} stable={len(stable)} | {details}"
                    )
                else:
                    self.get_logger().info("YOLO_BATANG detections=0 stable=0")
