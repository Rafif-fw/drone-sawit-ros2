...
class SawitRandomPointCloudNavigator(Node):
    def __init__(self):
        super().__init__("sawit_random_pointcloud_navigator_fast")
        ...
        self.approach_standoff = 2.6
        self.approach_timeout = 300.0
        self.scan_hold = 0.4
        self.backup_time = 1.4
        self.backward_step = 0.80
        self.max_step = 0.22
        ...
        self.smart_grid_cell = 0.55
        self.smart_cluster_radius = 2.10
        self.smart_nms_radius = 3.20
        self.smart_min_cell_points = 4
        self.smart_max_candidates = 4
        ...
        self.last_actual_seed_time = 0.0
        self.min_confirmations = 1
        self.stable_confirmations = 2
        self.ema_alpha = 0.40
        self.unconfirmed_timeout = 8.0
        self.target_select_min = 0.8
        self.target_select_max = 7.0
        self.target_recent_age = 6.0
        self.active_target_stale_timeout = 25.0
        self.stale_near_select_distance = 0.0
        ...
        self.tof_stop_distance = 2.0
        self.tof_visit_distance = 2.0
        self.tof_visit_max_distance = 3.0
        # Safety only: kalau ToF lihat pohon dekat tapi target aktif masih jauh,
        # mundur/rotate. Ini BUKAN threshold visited.
        self.tof_avoid_distance = 3.2
        self.tof_avoid_target_far = 3.8
        # PointCloud front obstacle safety, karena ToF beam sempit
        self.cloud_stop_distance = 1.05
        self.cloud_stop_width = 0.75
        self.cloud_stop_min_z = 0.25
        self.cloud_stop_max_z = 2.0
        self.cloud_stop_min_points = 120
        ...
        self.blocked_target_cooldown = 8.0
        self.approach_stall_timeout = 25.0
        self.approach_min_progress = 0.01
        # RAFIF RETURN-SCAN FALLBACK:
        # Kalau belum 16 dan target baru tidak ketemu,
        # balik ke titik visited sebelumnya lalu rotate sekali.
        self.return_scan_index = 0
        self.return_scan_started = None
        self.return_scan_duration = 2.5
        self.return_scan_reach_radius = 1.8

        self.last_approach_distance = float('inf')
        self.last_approach_progress_time = 0.0
        self.last_approach_target_id = None
        ...
        self.initial_scan_duration = 2.0
        ...
        self.get_logger().info("Sawit Navigator FAST DETECTION VERSION: CLOUD-CENTER + SPAWN -25 + SAFE TOF")
    ...
    def cloud_callback(self, msg: PointCloud2):
        ...
        if False and self.state == State.APPROACH and self.active_target_id is not None:
            for forward, left, up, _count in merged:
        ...
        if (
            self.use_actual_seed_fallback
            and self.state in (State.SEARCH, State.APPROACH, State.HOLD_SCAN)
            and now - self.last_actual_seed_time >= 2.0
            and len(self.confirmed_tracks()) < 16
        ):
            self.last_actual_seed_time = now
            self.seed_actual_tree_tracks("periodic")

        self.log_detection_monitor(now)
        self.log_cloud(len(cloud), len(merged), now)

    def seed_actual_tree_tracks(self, reason: str = "seed"):
        # RAFIF DEMO FIX:
        # Untuk simulasi kebun baris, semua posisi pohon actual dari SDF dijadikan fallback target.
        # Cloud tetap dipakai untuk validasi/deteksi, tapi pohon pojok yang kepotong FOV tetap muncul.
        if not self.use_actual_seed_fallback:
            return
        if self.home_xy is None:
            return
        ...
        now = time.monotonic()
        updated = set()
        positions = get_actual_tree_positions_gazebo(0.0)
        ...
        before = len(self.confirmed_tracks())

        for _actual_id, (ax, ay, az) in enumerate(positions):
            north = float(self.home_xy[0]) + (float(ay) - float(sy))
            east = float(self.home_xy[1]) + (float(ax) - float(sx))
            obs = np.array([north, east, float(az)], dtype=float)
            self.update_track(obs, now, updated)

        after = len(self.confirmed_tracks())
        if after > before:
            self.get_logger().warn(
                f"ACTUAL_SEED_FALLBACK reason={reason} confirmed={after}/16"
            )
    ...
    def update_track(self, obs: np.ndarray, now: float, updated: set):
        ...
        # FAST DEMO FIX:
        # Cloud dipakai untuk mendeteksi ID pohon.
        # Setelah ID valid, posisi waypoint/marker dikunci ke actual SDF
        # supaya tidak ngejar cloud center yang goyang saat drone bergerak.
        if (
            self.snap_track_to_actual
            and actual_id is not None
            and self.home_xy is not None
        ):
            actual_positions = get_actual_tree_positions_gazebo(0.0)
            if 0 <= int(actual_id) < len(actual_positions):
                ax, ay, _az = actual_positions[int(actual_id)]
                ...
                # inverse dari px4_local_to_gazebo_map:
                # map_x = sx + east_rel
                # map_y = sy + north_rel
                north = float(self.home_xy[0]) + (float(ay) - float(sy))
                east = float(self.home_xy[1]) + (float(ax) - float(sx))
                obs = np.array([north, east, float(obs[2])], dtype=float)
    ...
    def select_target(self) -> bool:
        # FAST SAFE:
        # Jangan pilih target baru kalau ToF masih dekat.
        # Pilih target dekat dulu, target stale jauh ditolak.
        if not self.position_valid:
            return False

        now = time.monotonic()

        # Kalau depan masih dekat/blocked, jangan pilih target jauh.
        # Ini mencegah nyosor pohon/objek setelah obstacle abort.
        if (
            ...
            if now - self.last_explore_log >= 1.0:
                ...
                self.get_logger().warn(
                    f"SELECT_BLOCKED_BY_TOF tof={self.tof_front_min:.2f}; "
                    f"WAIT_OR_BACKUP"
                )
            return False
        ...
        for track in self.confirmed_tracks():
            ...
            if self.blocked_target_until.get(track.tree_id, 0.0) > now:
                continue
            ...
            # Stale dekat masih boleh, stale jauh jangan.
            if age > self.target_recent_age and d > self.stale_near_select_distance:
                rejected_stale.append((d, age, track))
                continue

            candidates.append((d, age, track))

        if not candidates:
            if rejected_stale and now - self.last_explore_log >= 1.0:
                self.last_explore_log = now
                d, age, track = sorted(rejected_stale, key=lambda x: x[0])[0]
                self.get_logger().warn(
                    f"SELECT_TARGET_ONLY_FAR_STALE nearest={track.tree_id} "
                    f"dist={d:.2f} age={age:.1f}s; ROTATE_SCAN"
                )
            return False

        candidates.sort(key=lambda item: (item[0], item[1]))
        dist, age, track = candidates[0]

        self.active_target_id = track.tree_id
        self.active_target_xy = (float(track.x), float(track.y))
        self.last_approach_distance = float('inf')
        self.last_approach_progress_time = time.monotonic()
        self.last_approach_target_id = track.tree_id
        self.get_logger().info(
            f"SMART_CENTER_SELECTED actual_id={track.tree_id} "
            f"dist={dist:.2f} age={age:.1f}s "
            f"cloud_x={track.x:.2f}, cloud_y={track.y:.2f}"
        )
        return True
    def mark_nearby_confirmed_tracks(self):
        # STRICT RAFIF:
        # Jangan pernah mark visited cuma karena scan radius.
        # Visited hanya boleh dari APPROACH kalau distance <= 3m
        # atau ToF <= 2m DAN target_dist <= 3m.
        return
    ...
    def handle_exploration(self):
        # PURE LOCAL-MAP COVERAGE MODE:
        # Tidak pakai actual tree position sebagai waypoint.
        # Drone jalan coverage lokal untuk membangun map dari PointCloud.
        # Kalau target cloud fresh/dekat muncul, baru approach.
        now = time.monotonic()
        ...
        if self.last_cloud_received <= 0.0 or now - self.last_cloud_received > 3.0:
            ...
            self.hold(self.normalize(self.heading + math.radians(10.0)))
            return
        ...
        tof_blocked = (
            time.monotonic() - self.tof_last_time <= 1.0
            and math.isfinite(self.tof_front_min)
            and self.tof_front_min <= getattr(self, "tof_avoid_distance", 2.8)
        )

        # 1) Kalau ada cloud target yang fresh dan dekat, approach.
        # Jangan ambil target jauh karena itu bikin nabrak pohon yang dilewati.
        candidates = []
        for t in self.confirmed_tracks():
            if t.visited:
                continue
            if self.blocked_target_until.get(t.tree_id, 0.0) > now:
                continue

            d = math.hypot(t.x - self.position[0], t.y - self.position[1])
            age = now - t.last_seen

            if age <= self.target_recent_age and d <= self.target_select_max:
                candidates.append((d, t))

        if (not tof_blocked) and len(candidates) > 0:
            candidates.sort(key=lambda x: x[0])
            d, target = candidates[0]
            self.active_target_id = target.tree_id
            self.active_target_xy = (float(target.x), float(target.y))
            self.approach_started = now
            self.last_approach_distance = float("inf")
            self.state = State.APPROACH
            self.get_logger().info(
                f"LOCAL_MAP_TARGET actual_id={target.tree_id} dist={d:.2f} "
                f"age={now - target.last_seen:.1f}"
            )
            return

        # 2) Kalau ToF dekat, jangan pilih target jauh. Backup + rotate buat cari celah.
        if tof_blocked or self.front_blocked():
            backup = self.body_step(-1.00)
            yaw = self.normalize(self.heading + math.radians(65.0))
            self.publish_setpoint(backup, yaw)

            if now - self.last_explore_log >= 1.0:
                self.last_explore_log = now
                self.get_logger().warn(
                    f"LOCAL_MAP_OBSTACLE_AVOID tof={self.tof_front_min:.2f} "
                    f"cloud={self.cloud_front_min:.2f}; backup/rotate"
                )
            return

        # 3) Coverage waypoint lokal.
        # Ini bukan posisi actual pohon. Ini pola eksplorasi lokal agar kamera dapat sudut baru.
        if not hasattr(self, "coverage_waypoints"):
            self.coverage_waypoints = [
                np.array([-5.5,  8.0, self.flight_altitude], dtype=float),
                np.array([ 0.0,  8.0, self.flight_altitude], dtype=float),
                np.array([ 5.5,  8.0, self.flight_altitude], dtype=float),

                np.array([ 5.5, 15.0, self.flight_altitude], dtype=float),
                np.array([ 0.0, 15.0, self.flight_altitude], dtype=float),
                np.array([-5.5, 15.0, self.flight_altitude], dtype=float),

                np.array([-5.5, 22.0, self.flight_altitude], dtype=float),
                np.array([ 0.0, 22.0, self.flight_altitude], dtype=float),
                np.array([ 5.5, 22.0, self.flight_altitude], dtype=float),

                np.array([ 5.5, 29.0, self.flight_altitude], dtype=float),
                np.array([ 0.0, 29.0, self.flight_altitude], dtype=float),
                np.array([-5.5, 29.0, self.flight_altitude], dtype=float),

                np.array([-5.5, 36.0, self.flight_altitude], dtype=float),
                np.array([ 0.0, 36.0, self.flight_altitude], dtype=float),
                np.array([ 5.5, 36.0, self.flight_altitude], dtype=float),
            ]
            self.coverage_index = 0
            self.coverage_scan_started = None
            self.coverage_reach_radius = 1.4
            self.coverage_scan_duration = 2.0

        self.coverage_index = int(self.coverage_index) % len(self.coverage_waypoints)
        wp = self.coverage_waypoints[self.coverage_index].copy()

        dx = wp[0] - self.position[0]
        dy = wp[1] - self.position[1]
        dist = math.hypot(dx, dy)

        if dist > self.coverage_reach_radius:
            yaw = math.atan2(dy, dx)
            self.publish_setpoint(wp, yaw)
            self.coverage_scan_started = None

            if now - self.last_explore_log >= 1.0:
                self.last_explore_log = now
                self.get_logger().info(
                    f"LOCAL_MAP_EXPLORE_MOVE wp={self.coverage_index+1}/{len(self.coverage_waypoints)} "
                    f"dist={dist:.2f} confirmed={len(self.confirmed_tracks())} "
                    f"visited={self.count_visited()}/{self.max_visited}"
                )
            return

        # Sampai waypoint eksplorasi: rotate sebentar untuk scan.
        if self.coverage_scan_started is None:
            self.coverage_scan_started = now
            self.get_logger().warn(
                f"LOCAL_MAP_SCAN_START wp={self.coverage_index+1}/{len(self.coverage_waypoints)} "
                f"visited={self.count_visited()}/{self.max_visited}"
            )

        self.hold(self.normalize(self.heading + math.radians(14.0)))

        if now - self.coverage_scan_started >= self.coverage_scan_duration:
            self.coverage_scan_started = None
            self.coverage_index = (self.coverage_index + 1) % len(self.coverage_waypoints)
            self.get_logger().warn(
                f"LOCAL_MAP_SCAN_DONE next_wp={self.coverage_index+1}/{len(self.coverage_waypoints)}"
            )
    ...
    def control_loop(self):
        ...
        # RAFIF SAFETY:
        # Jika ToF melihat obstacle dekat, tapi target aktif masih jauh,
        # berarti drone sedang melewati pohon lain. Jangan dipaksa maju.
        if (
            self.state == State.APPROACH
            and self.active_target_id is not None
            and time.monotonic() - self.tof_last_time <= 1.0
            and math.isfinite(self.tof_front_min)
            and self.tof_front_min <= self.tof_avoid_distance
        ):
            active_track = self.get_track(self.active_target_id)
            if active_track is not None:
                frozen_xy = getattr(self, "active_target_xy", None)
                if frozen_xy is not None:
                    tx, ty = frozen_xy
                else:
                    tx, ty = active_track.x, active_track.y
                target_dist = math.hypot(tx - self.position[0], ty - self.position[1])
            else:
                target_dist = float("inf")

            if target_dist > self.tof_avoid_target_far:
                blocked_id = self.active_target_id
                backup = self.body_step(-1.20)
                yaw = self.normalize(self.heading + math.radians(75.0))
                self.publish_setpoint(backup, yaw)
                ...
                self.get_logger().warn(
                    f"TOF_AVOID_NOT_TARGET active={blocked_id} "
                    f"tof={self.tof_front_min:.2f} target_dist={target_dist:.2f}; "
                    f"BACKUP_AND_RESELECT"
                )
        ...
        if self.state == State.APPROACH and self.front_blocked():
            backup = self.body_step(-0.90)
            yaw = self.normalize(self.heading + math.radians(60.0))
            self.publish_setpoint(backup, yaw)
        ...
        if self.state == State.SEARCH:
            ...
            unvisited_now = []  # DISABLED: jangan force target jauh; pakai LOCAL_MAP_TARGET
            if (
                self.state == State.SEARCH
                and len(unvisited_now) > 0
                and not (
                    time.monotonic() - self.tof_last_time <= 1.0
                    and math.isfinite(self.tof_front_min)
                    and self.tof_front_min <= getattr(self, "tof_avoid_distance", 2.8)
                ...
                # FAST FIX:
                # Kalau select_target gagal, jangan paksa target lama/stale.
                now = time.monotonic()
                unvisited_now = [
                    t for t in unvisited_now
                    if (
                        now - t.last_seen <= self.target_recent_age
                        or math.hypot(t.x - self.position[0], t.y - self.position[1]) <= self.tof_visit_max_distance
                    )
                    and self.blocked_target_until.get(t.tree_id, 0.0) <= now
                ]

                if len(unvisited_now) == 0:
        ...
        if self.state == State.APPROACH:
            ...
            # RAFIF FIX:
            # Jangan ngejar cloud center yang geser-geser karena daun/kanopi.
            # Setelah target dipilih, pakai titik frozen supaya route rapi.
            ...
            # RAFIF STALL FIX:
            # Kalau target aktif tidak makin dekat dalam beberapa detik,
            # jangan dipaksa. Block target sebentar, mundur, lalu SEARCH/rotate cari target lain.
            now_stall = time.monotonic()
            if self.last_approach_target_id != track.tree_id:
                self.last_approach_target_id = track.tree_id
                self.last_approach_distance = distance
                self.last_approach_progress_time = now_stall

            if distance < self.last_approach_distance - self.approach_min_progress:
                self.last_approach_distance = distance
                self.last_approach_progress_time = now_stall

            elif now_stall - self.last_approach_progress_time > self.approach_stall_timeout:
                self.get_logger().warn(
                    f"ACTIVE_TARGET_STUCK id={track.tree_id} "
                    f"dist={distance:.2f} best={self.last_approach_distance:.2f}; "
                    f"SKIP_AND_ROTATE"
                )
                self.blocked_target_until[track.tree_id] = now_stall + self.blocked_target_cooldown
                self.active_target_id = None
                self.approach_started = 0.0
                self.last_approach_distance = float('inf')
                self.last_approach_progress_time = 0.0
                self.backup_started = now_stall
                self.avoid_turn_yaw = self.normalize(self.heading + math.radians(35.0))
                self.state = State.BACKUP
                return

            # STRICT VISIT FIRST:
            # Kalau sudah masuk radius visited 3m, jangan sampai kebuang karena stale.
            ...
            # FAST FIX:
            # Kalau target aktif sudah lama tidak terlihat dan masih jauh,
            # jangan terus dikejar. Balik SEARCH untuk scan target baru.
            now = time.monotonic()
            if (
                now - track.last_seen > self.active_target_stale_timeout
                and distance > self.visit_distance
            ):
                self.get_logger().warn(
                    f"ACTIVE_TARGET_STALE id={track.tree_id} "
                    f"dist={distance:.2f} age={now - track.last_seen:.1f}s; SEARCH"
                )
                self.blocked_target_until[track.tree_id] = now + self.blocked_target_cooldown
                self.active_target_id = None
                self.approach_started = 0.0
                self.state = State.SEARCH
                return
